import { Component, OnInit, ViewChild, ViewContainerRef } from '@angular/core';
import { Router } from '@angular/router';
import { ModalComponent } from 'ng2-bs3-modal/ng2-bs3-modal';
import { ToastsManager } from 'ng2-toastr/ng2-toastr';
import { AllocatedFromService } from './allocatedfrom.service';
import { GridOptions } from 'ag-grid';
import { IAllocatedFrom } from './allocatedfrom';

@Component({
    moduleId: module.id,
    templateUrl: 'list_allocatedfromeng.component.html'
})
export class List_allocatedfromengComponent implements OnInit {
  private Table_652GridOptions: GridOptions;

  private allocatedfrom: IAllocatedFrom = {
  	id: 0,
  	name: '',
  	description: ''}
  ;

array_AllocatedFrom :any[];
  constructor(private router: Router, public toastr: ToastsManager, vcr: ViewContainerRef, private allocatedfromservice: AllocatedFromService) { 
    this.toastr.setRootViewContainerRef(vcr);
    this.Table_652GridOptions = {
    	columnDefs: [
    		{
    			headerName: "Name",
    			field: "name",
    		},
    		{
    			headerName: "Description",
    			field: "description",
    		}
    	],
    	rowSelection: 'single',
    	onRowClicked: function(event) {
    		console.log('a row was clicked');
    	}
    };

  }

  ngOnInit() {
    this.get_all_AllocatedFrom();
  }


  get_all_AllocatedFrom(){
      this.allocatedfromservice.get_all_AllocatedFrom()
          .subscribe(data => {
            this.array_AllocatedFrom =data;
            console.log("array_AllocatedFrom returned");
            //this.toastr.success('Success!');
          },
          error => {
            this.toastr.error('Check the browser console to see more info.','Error!');
          });
  }

}